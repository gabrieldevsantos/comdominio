...
@Controller
@RequestMapping(value = "/tipoarealazer")
public class TipoAreaLazerController extends DaoImplementacao<TipoAreaLazer>
		implements DaoInterface<TipoAreaLazer> {
	
	public TipoAreaLazerController(Class<TipoAreaLazer> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		TipoAreaLazer objeto = new Gson().fromJson(jsonPost,
				TipoAreaLazer.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		TipoAreaLazer objeto = new Gson().fromJson(jsonPut, TipoAreaLazer.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		TipoAreaLazer objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
