...
import com.comdomino.model.Servico;
import com.google.gson.Gson;

@Controller
@RequestMapping(value = "/servico")
public class ServicoController extends DaoImplementacao<Servico>
		implements DaoInterface<Servico> {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		Servico objeto = new Gson().fromJson(jsonPost,
				Servico.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		Servico objeto = new Gson().fromJson(jsonPut, Servico.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		Servico objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
