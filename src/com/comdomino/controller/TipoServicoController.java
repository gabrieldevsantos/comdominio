...
public class TipoServicoController extends DaoImplementacao<TipoServico>
		implements DaoInterface<TipoServico> {
	
	public TipoServicoController(Class<TipoServico> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		TipoServico objeto = new Gson().fromJson(jsonPost,
				TipoServico.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		TipoServico objeto = new Gson().fromJson(jsonPut, TipoServico.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		TipoServico objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
