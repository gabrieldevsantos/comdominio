...
public class TipoUnidadeController extends DaoImplementacao<TipoUnidade>
		implements DaoInterface<TipoUnidade> {
	
	public TipoUnidadeController(Class<TipoUnidade> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		TipoUnidade objeto = new Gson().fromJson(jsonPost,
				TipoUnidade.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		TipoUnidade objeto = new Gson().fromJson(jsonPut, TipoUnidade.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		TipoUnidade objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
