...
public class CondominioController extends DaoImplementacao<Condominio>
		implements DaoInterface<Condominio> {
	
	public CondominioController(Class<Condominio> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		Condominio objeto = new Gson().fromJson(jsonPost,
				Condominio.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		Condominio objeto = new Gson().fromJson(jsonPut, Condominio.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		Condominio objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
