...
public class NotificacaoController extends DaoImplementacao<Notificacao>
		implements DaoInterface<Notificacao> {
	
	public NotificacaoController(Class<Notificacao> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		Notificacao objeto = new Gson().fromJson(jsonPost,
				Notificacao.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		Notificacao objeto = new Gson().fromJson(jsonPut, Notificacao.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		Notificacao objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
