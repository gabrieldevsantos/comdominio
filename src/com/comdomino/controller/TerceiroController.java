...
public class TerceiroController extends DaoImplementacao<Terceiro>
		implements DaoInterface<Terceiro> {
	
	public TerceiroController(Class<Terceiro> persistenceClass) {
	...
	public ResponseEntity salvar(@RequestBody String jsonPost)
			throws Exception{
		Terceiro objeto = new Gson().fromJson(jsonPost,
				Terceiro.class);
	...
	public ResponseEntity atualizar(@RequestBody String jsonPut)
			throws Exception{
		Terceiro objeto = new Gson().fromJson(jsonPut, Terceiro.class);
	...
	String buscar(@PathVariable("id") String id)
			throws Exception {
		Terceiro objeto = super.loadObjeto(Long.parseLong(id));
		if (objeto == null) {
			return "{}";
		}
